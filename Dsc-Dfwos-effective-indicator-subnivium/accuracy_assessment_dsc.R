...
# ghcn
data<-load('./assessment_mod_avh_seperate/ghcn_dws.RData')
data.all<-data.frame(dataset=1,validation.df)
# china
data<-load('./assessment_mod_avh_seperate/china_dws.RData')
data.all<-rbind(data.all,data.frame(dataset=2,validation.df))
# russia
data<-load('./assessment_mod_avh_seperate/russia_dws.RData')
data.all<-rbind(data.all,data.frame(dataset=3,validation.df))
# snotel
data<-load('./assessment_mod_avh_seperate/snotel_dws.RData')
...
c(rmse.40.less,rmse.40.45,rmse.45.50,rmse.50.more,rmse.all)
####################################################
# BY ELEVATION
...
c(rmse.0.300,rmse.300.600,rmse.600.900,rmse.900.more)
...
data.all<-data.all[!((data.all$stn>150) & (data.all$stn<250)&(data.all$rs>0) & (data.all$rs<30)),]
...
p3 <- ggplot(data.all) + 
  ...
  xlab(expression(D[sc]~(station-based)))+
  ylab(expression(D[sc]~(remote~sensing-based)))+
  ...
  annotate("text", label = "R=0.90, Slope=0.80", x = 80, y = 250, size = 2.5, colour = "black")

p3

output.filename<-paste(output.folder,'/all_datasets_dws.tiff',sep = '')
