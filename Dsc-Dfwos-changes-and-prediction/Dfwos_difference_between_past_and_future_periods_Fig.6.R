...
{
  file.path<-paste(getwd(),'/metrics_05degree_nh/','dfwos',toString(y),'.tif',sep = '')
...
{
  file.path<-paste(getwd(),'/prediction_dws_dwos/aggregation/dwos_nh/','mean_dwos',toString(y),'.tif',sep = '')
  ras.obj<-raster(file.path)
  if (index==1) 
  {
    proj.info<-projection(ras.obj)
...
writeRaster(future.ras,filename=file.path(outputFolder,"dif_dwos_nh_past_future.tif"),format='GTiff',overwrite=TRUE)