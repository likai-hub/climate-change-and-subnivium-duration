...
{
  file.path<-paste(getwd(),'/avhrr_82_07_mod_02_08_14_nh/','dfws',toString(y),'.tif',sep = '')
    ...
    } else
    {
      ...
      } else
      {
        if (sum(pixel.values>=0)>32*0.8 ){
          ...
          if (is.na(cse.val) | (cse.val>(-1.96) & cse.val<1.96)){
            mat.c.test[r,c]<-0
...
outputFolder<-'E:/backup2/new_processing_12_30_2017/statistical_analysis/trend_results_revision_2'
if (! dir.exists(outputFolder)){dir.create(outputFolder)}

# save matrix to raster
# slope
c.ras<-raster(mat.c,xmn=xmin.val,xmx=xmax.val,ymn=ymin.val,ymx=ymax.val,crs=proj.info)
writeRaster(c.ras,filename=file.path(outputFolder,"slp_dws_nh.tif"),format='GTiff',overwrite=TRUE)
# C/SE(C)
c.se.ras<-raster(mat.c.se,xmn=xmin.val,xmx=xmax.val,ymn=ymin.val,ymx=ymax.val,crs=proj.info)
writeRaster(c.se.ras,filename=file.path(outputFolder,"c_se_dws_nh.tif"),format='GTiff',overwrite=TRUE)

# Likelihood ratio test
lrt.ras<-raster(mat.lrt,xmn=xmin.val,xmx=xmax.val,ymn=ymin.val,ymx=ymax.val,crs=proj.info)
writeRaster(lrt.ras,filename=file.path(outputFolder,"lrt_dws_nh.tif"),format='GTiff',overwrite=TRUE)


# slope with test
slope.ras<-raster(mat.c.test,xmn=xmin.val,xmx=xmax.val,ymn=ymin.val,ymx=ymax.val,crs=proj.info)
writeRaster(slope.ras,filename=file.path(outputFolder,"slope_dws_nh.tif"),format='GTiff',overwrite=TRUE)
...
writeRaster(slope.ras.sig,filename=file.path(outputFolder,"slope_dws_nh_sig.tif"),format='GTiff',overwrite=TRUE)
