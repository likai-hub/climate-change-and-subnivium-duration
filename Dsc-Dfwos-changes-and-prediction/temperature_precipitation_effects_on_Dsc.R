...
writeRaster(ras,filename=file.path(outputFolder,"coef_temp_test_dsc_nh.tif"),format='GTiff',overwrite=TRUE)

# coef.pre<-coef.pre/10.0
ras<-raster(coef.pre.test,xmn=xmin.val,xmx=xmax.val,ymn=ymin.val,ymx=ymax.val,crs=proj.info)
writeRaster(ras,filename=file.path(outputFolder,"coef_pre_test_dsc_nh.tif"),format='GTiff',overwrite=TRUE)
