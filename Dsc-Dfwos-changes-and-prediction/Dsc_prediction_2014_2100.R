...
outputPath1<-paste(outputFolder,'/lrt/dws_nh',sep='')
...
for (y in seq(1982,2014)){
  ...
  if (index == 1){
    ...
    file.path<-paste(folder.snow.vars,'/dfws',toString(y),'.tif',sep='')
...
for (fd in folderList)
{
  ...
  outputPath22<-paste(outputPath2,'/',file.name,'/dws',sep = '')
  if (! dir.exists(outputPath22)) {dir.create(outputPath22,recursive = TRUE)}
  if (file.exists(file.path(outputPath22,paste('dws',as.character(2098),".tif",sep = ''))))
  {
    next
  }
  index<-1
  ...
  for (r in 1:rows){
    ...
    print(paste(file.name,':',as.character(r),sep = ''))
  }
  ...
  for (l in 1:dim(newx)[3])
  {
    ...
    writeRaster(ras,filename=file.path(outputPath22,paste('dws',as.character(2014+l),".tif",sep = '')),format='GTiff',overwrite=TRUE)
  }
